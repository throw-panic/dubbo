...
/**
 *  todo: 实现 BeanDefinitionParser 接口，Dubbo Bean 定义解析器。
 */
...
public class DubboBeanDefinitionParser implements BeanDefinitionParser {
     ...
     * todo:
     *      beanClass ，Bean 对象的类。
     *      required ，是否需要在 Bean 对象的编号( id ) 不存在时，自动生成编号。
    ...
    private static BeanDefinition parse(Element element, ParserContext parserContext, Class<?> beanClass, boolean required) {
        // 创建 RootBeanDefinition, 并且set beanClass 等参数值
         ...
         */
        beanDefinition.setLazyInit(false);  // 默认 lazyInit 为 false
        ...
        if (ProtocolConfig.class.equals(beanClass)) {
            ...
            if (className != null && className.length() > 0) {
                // 创建 Service 的 RootBeanDefinition 对象。相当于内嵌了
                // <bean class="com.alibaba.dubbo.demo.provider.DemoServiceImpl" />
                // 注意： 一般不这么用（处理 <dubbo:service class="" />）
        ...
        // 循环 Bean 对象的setting 方法，将属性添加到Bean 对象的属性赋值
    ...
    private static void parseNested(Element element, ParserContext parserContext, Class<?> beanClass, boolean required, String tag, String property, String ref, BeanDefinition beanDefinition) {
        ...
        if (nodeList != null && nodeList.getLength() > 0) {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                /**
                 * todo:
                 *      只解析指定标签。目前有内嵌的 <dubbo:service /> 和 <dubbo:reference /> 标签。
                 *      下面两个 if 判断就是做这件事情：判断是否为指定要解析的子元素。
                 */
